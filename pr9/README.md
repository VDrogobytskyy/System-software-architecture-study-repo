...
Для виконання цього завдання було розроблено скрипт, який ідентифікує тип користувача за допомогою аналізу файлів /etc/passwd та /etc/shadow. Замість стандартної перевірки номера UID, програма досліджує три непрямі ознаки: наявність командної оболонки для входу в термінал, розташування домашньої директорії (поза межами /home) та статус пароля (чи він заблокований для входу). Такий комплексний підхід дозволяє точно визначити роль акаунта в системі, навіть якщо його цифрові ідентифікатори були змінені адміністратором.

Отриманий вивід підтверджує, що обліковий запис є звичайним (REGULAR), оскільки він має активну оболонку /bin/bash та стандартну домашню папку, що характерно для реального користувача. Статус пароля "P" (Passable) вказує на те, що акаунт відкритий для авторизації, на відміну від системних сервісів, які зазвичай мають статус "L" (Locked) та обмежений доступ до середовища виконання. Це доводить, що безпека Linux базується на функціональних обмеженнях кожного окремого профілю.

Вивід в консоль:
```bash
./task_by_var_6.sh.x drogobytskyyy
Analyzing user: drogobytskyyy
[sudo] password for drogobytskyyy:

Shell      : /bin/bash
Home       : /home/drogobytskyyy
Password   : P (L=Locked, P=Passable)
