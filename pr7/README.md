...
Відповідь:
Обгортаємо виклик нашої функції в збереження параметрів типу clock_t, тобто записуєм час перед виконанням функції та після, після чого віднімаємо і отримуємо результат.

Вивід в консоль:
```bash
./task9

Our func worked for: 1.2350 miliseconds.
```
